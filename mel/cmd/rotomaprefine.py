"""Refine mole positions using template matching.

Normalized cross-correlation is used to find the best match. Crops a
template around each canonical mole in the reference image, then slides
it over a local search region in the target image to find the best
...
def _load_image_with_mask(image_path):
    ...
    if mask is not None:
        ...
        mean_val = 128
        for c in range(3):
            image_bgr[:, :, c] = np.where(keep_mask, image_bgr[:, :, c], mean_val)
    return image_bgr
...
def _normalize_resolution(images_with_paths):
    ...
    smallest_max_dim = min(max(img.shape[:2]) for _, img in images_with_paths)
...
def process_args(args):
    ...
    for mole_uuid, (ref_path, ref_mole) in ref_canonical_by_uuid.items():
        ...
        if (
            template.shape[0] < template_size // 2
            or template.shape[1] < template_size // 2
        ):
            continue
