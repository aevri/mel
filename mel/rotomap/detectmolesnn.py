...
class Conv3x3HueSatMaskMxyNext(Model2):
    def __init__(self, total_steps, use_onecycle=True):
        ...
        self.cnn = torch.nn.Sequential(
            ...
            MxyNextModule(width, width * 2),
            MxyNextModule(width * 2, width * 2, is_pointwise=True),
            MxyNextModule(width * 2, width * 4),
            MxyNextModule(width * 4, width * 4, is_pointwise=True),
            MxyNextModule(width * 4, width * 4, is_pointwise=True),
            MxyNextModule(width * 4, width * 4, is_pointwise=True),
            MxyNextModule(width * 4, width * 4, is_pointwise=True),
            MxyNextModule(width * 4, 3, is_pointwise=True, use_swish=False),
            torch.nn.Sigmoid(),
        )
